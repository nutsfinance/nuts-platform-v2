...
contract SpotSwap is InstrumentBase {
    ...
    // Custom data
    bytes32 internal constant SWAP_DATA = "swap_data";
    ...
    function createIssuance(
        ...
        SpotSwapMakerParameters.Data memory makerParameters = SpotSwapMakerParameters
            .decode(makerParametersData);

        // Validates parameters.
        require(
            makerParameters.inputTokenAddress != address(0x0),
            "Input token not set"
        );
        require(
            makerParameters.outputTokenAddress != address(0x0),
        ...
        require(
            makerParameters.duration >= DURATION_MIN &&
                makerParameters.duration <= DURATION_MAX,
            "Invalid duration"
        );

        // Validate input token balance
        uint256 inputTokenBalance = EscrowBaseInterface(
            _instrumentEscrowAddress
        )
            .getTokenBalance(callerAddress, makerParameters.inputTokenAddress);
        require(
            inputTokenBalance >= makerParameters.inputAmount,
    ...
    function engageIssuance(
        ...
        uint256 outputTokenBalance = EscrowBaseInterface(
            _instrumentEscrowAddress
        )
            .getTokenBalance(callerAddress, _outputTokenAddress);
        require(
            outputTokenBalance >= _outputAmount,
            "Insufficient output balance"
        );
    ...
    function getCustomData(
        ...
        if (dataName == SWAP_DATA) {
            SpotSwapProperties.Data memory spotSwapProperties = SpotSwapProperties
                .Data({
            ...
            SpotSwapCompleteProperties.Data memory spotSwapCompleteProperties = SpotSwapCompleteProperties
                .Data({
            ...
            return
                SpotSwapCompleteProperties.encode(spotSwapCompleteProperties);
        } else {
            revert("Unknown data");
        }
    }
