...
library InstrumentParameters {
   ...
   * @return The number of bytes decoded
   */
  function _read_expiration(uint p, bytes memory bs, Data memory r, uint[4] memory counters) internal pure returns (uint) {
    /**
   ...
   * @return The number of bytes decoded
   */
  function _read_supportMakerWhitelist(uint p, bytes memory bs, Data memory r, uint[4] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
   ...
   * @return The number of bytes decoded
   */
  function _read_supportTakerWhitelist(uint p, bytes memory bs, Data memory r, uint[4] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (bool x, uint sz) = ProtoBufRuntime._decode_bool(p, bs);
