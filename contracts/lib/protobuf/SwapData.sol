...
library SpotSwapData {
  ...
  function _decode(uint p, bytes memory bs, uint sz)
    ...
    while(pointer < offset+sz) {
      ...
      if(fieldId == 1) {
        ...
        pointer += _read_swapDueTimestamp(pointer, bs, r, counters);
      }
      else if(fieldId == 7) {
   ...
   * @return The number of bytes decoded
   */
  function _read_inputTokenAddress(uint p, bytes memory bs, Data memory r, uint[13] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (address x, uint sz) = ProtoBufRuntime._decode_sol_address(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_outputTokenAddress(uint p, bytes memory bs, Data memory r, uint[13] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (address x, uint sz) = ProtoBufRuntime._decode_sol_address(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_inputAmount(uint p, bytes memory bs, Data memory r, uint[13] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_outputAmount(uint p, bytes memory bs, Data memory r, uint[13] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_duration(uint p, bytes memory bs, Data memory r, uint[13] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_swapDueTimestamp(uint p, bytes memory bs, Data memory r, uint[13] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_escrowAddress(uint p, bytes memory bs, Data memory r, uint[13] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (address x, uint sz) = ProtoBufRuntime._decode_sol_address(p, bs);
  ...
  function _encode(Data memory r, uint p, bytes memory bs)
    ...
    pointer += ProtoBufRuntime._encode_sol_uint256(r.swapDueTimestamp, pointer, bs);
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.outputAmount = input.outputAmount;
    output.duration = input.duration;
    output.swapDueTimestamp = input.swapDueTimestamp;
