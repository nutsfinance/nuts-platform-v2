...
library LendingProperties {
   ...
   * @return The number of bytes decoded
   */
  function _read_lendingTokenAddress(uint p, bytes memory bs, Data memory r, uint[9] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (address x, uint sz) = ProtoBufRuntime._decode_sol_address(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_lendingAmount(uint p, bytes memory bs, Data memory r, uint[9] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
...
library LendingCompleteProperties {
   ...
   * @return The number of bytes decoded
   */
  function _read_lendingProperties(uint p, bytes memory bs, Data memory r, uint[3] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (LendingProperties.Data memory x, uint sz) = _decode_LendingProperties(p, bs);
    if(isNil(r)) {
      counters[2] += 1;
    } else {
   ...
   * @return The number of bytes used to decode
   */
  function _decode_LendingProperties(uint p, bytes memory bs)
      internal pure returns (LendingProperties.Data memory, uint) {
    uint pointer = p;
    (uint sz, uint bytesRead) = ProtoBufRuntime._decode_varint(pointer, bs);
    pointer += bytesRead;
