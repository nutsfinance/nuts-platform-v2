...
library BorrowingData {
  ...
  function _decode(uint p, bytes memory bs, uint sz)
    ...
    while(pointer < offset+sz) {
      ...
      if(fieldId == 1) {
        ...
        pointer += _read_borrowingDueTimestamp(pointer, bs, r, counters);
      }
      else if(fieldId == 10) {
    ...
    while(pointer < offset+sz) {
      ...
      pointer += bytesRead;
      if(fieldId == 1) {
        pointer += _read_borrowingTokenAddress(pointer, bs, nil(), counters);
      }
      else if(fieldId == 2) {
        pointer += _read_collateralTokenAddress(pointer, bs, nil(), counters);
      }
      else if(fieldId == 3) {
        pointer += _read_borrowingAmount(pointer, bs, nil(), counters);
      }
      else if(fieldId == 4) {
        ...
        pointer += _read_borrowingDueTimestamp(pointer, bs, nil(), counters);
      }
      else if(fieldId == 10) {
   ...
   * @return The number of bytes decoded
   */
  function _read_borrowingTokenAddress(uint p, bytes memory bs, Data memory r, uint[18] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (address x, uint sz) = ProtoBufRuntime._decode_sol_address(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_borrowingAmount(uint p, bytes memory bs, Data memory r, uint[18] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
   ...
   * @return The number of bytes decoded
   */
  function _read_borrowingDueTimestamp(uint p, bytes memory bs, Data memory r, uint[18] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
  ...
  function _encode(Data memory r, uint p, bytes memory bs)
    ...
    pointer += ProtoBufRuntime._encode_sol_uint256(r.borrowingDueTimestamp, pointer, bs);
  ...
  function store(Data memory input, Data storage output) internal {
    ...
    output.borrowingDueTimestamp = input.borrowingDueTimestamp;
