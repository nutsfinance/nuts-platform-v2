...
library InstrumentStatus {
  ...
  function _decode(uint p, bytes memory bs, uint sz)
    ...
    while(pointer < offset+sz) {
      ...
      pointer += bytesRead;
      if(fieldId == 1) {
        pointer += _read_instrumentManagerAddress(pointer, bs, r, counters);
      }
      else if(fieldId == 2) {
        pointer += _read_fspAddress(pointer, bs, r, counters);
      }
      else if(fieldId == 3) {
        pointer += _read_activeness(pointer, bs, r, counters);
      }
      else if(fieldId == 4) {
        pointer += _read_creation(pointer, bs, r, counters);
      }
      else if(fieldId == 5) {
        pointer += _read_tokenDeposited(pointer, bs, r, counters);
      }
      else {
   ...
   * @return The number of bytes decoded
   */
  function _read_instrumentManagerAddress(uint p, bytes memory bs, Data memory r, uint[6] memory counters) internal pure returns (uint) {
    /**
   ...
   * @return The number of bytes decoded
   */
  function _read_activeness(uint p, bytes memory bs, Data memory r, uint[6] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
   ...
   * @return The number of bytes decoded
   */
  function _read_tokenDeposited(uint p, bytes memory bs, Data memory r, uint[6] memory counters) internal pure returns (uint) {
    /**
     * if `r` is NULL, then only counting the number of fields.
     */
    (uint256 x, uint sz) = ProtoBufRuntime._decode_sol_uint256(p, bs);
  ...
  function _encode(Data memory r, uint p, bytes memory bs)
    ...
    pointer += ProtoBufRuntime._encode_sol_address(r.instrumentManagerAddress, pointer, bs);
    ...
    pointer += ProtoBufRuntime._encode_sol_uint8(r.activeness, pointer, bs);
    ...
    pointer += ProtoBufRuntime._encode_sol_uint256(r.tokenDeposited, pointer, bs);
  ...
  function store(Data memory input, Data storage output) internal {
    output.instrumentManagerAddress = input.instrumentManagerAddress;
    output.fspAddress = input.fspAddress;
    output.activeness = input.activeness;
    output.creation = input.creation;
    output.tokenDeposited = input.tokenDeposited;
