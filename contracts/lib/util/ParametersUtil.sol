...
contract ParametersUtil {
     ...
     */
    function getBorrowingMakerParameters(address collateralTokenAddress, address borrowingTokenAddress, uint256 borrowingAmount,
        uint32 collateralRatio, uint32 tenorDays, uint32 interestRate) public pure returns (bytes memory) {
        BorrowingMakerParameters.Data memory borrowingMakerParamaters = BorrowingMakerParameters.Data(collateralTokenAddress,
            borrowingTokenAddress, borrowingAmount, collateralRatio, tenorDays, interestRate);
     ...
     */
    function getSpotSwapMakerParameters(address inputTokenAddress, address outputTokenAddress, uint256 inputAmount,
        uint256 outputAmount, uint256 duration) public pure returns (bytes memory) {
        SpotSwapMakerParameters.Data memory spotSwapMakerParameters = SpotSwapMakerParameters.Data(inputTokenAddress,
            outputTokenAddress, inputAmount, outputAmount, duration);
